...
urlpatterns = format_suffix_patterns([
     ...
     url(r'^folio/(?P<pk>[0-9]+)/$', FolioDetail.as_view(), name="folio-detail"),
     # Chants
     ...
     url(r'^concordance/(?P<pk>[0-9]+)/$', ConcordanceDetail.as_view(),name="concordance-detail"),
     #######################
     # Direct Solr queries #
     #######################
     ...
     url(r'^chant-set/manuscript/(?P<pk>[0-9]+)/$', ManuscriptChantSetView.as_view(), name="manuscript-chant-set-view"),
     url(r'^chant-set/manuscript/(?P<pk>[0-9]+)/page-(?P<start>[0-9]+)/$', ManuscriptChantSetView.as_view(), name="manuscript-chant-set-view-page"),
     url(r'^manuscript/(?P<pk>[0-9]+)/glyph-set$', ManuscriptGlyphSetView.as_view(), name="manuscript-glyph-set-view-page"),
     # Query Folios by manuscript
     url(r'^folio-set/manuscript/(?P<pk>[0-9]+)/$', ManuscriptFolioSetView.as_view(), name="manuscript-folio-set-view"),
     url(r'^folio-set/manuscript/(?P<pk>[0-9]+)/(?P<image_uri>([!#$&-;=?-[\]_a-zA-Z0-9~]|%[0-9a-fA-F]{2})+)/$', ManuscriptFolioSetView.as_view(), name="manuscript-folio-set-view-index"),
     # Search
     ...
     url(r'^manifest-proxy/(?P<manifest_url>([!#$&-;=?-[\]_a-zA-Z0-9~]|%[0-9a-fA-F]{2})+)/$', ManifestProxyView.as_view(), name="retrieve-manifest"),
     # Admin pages
