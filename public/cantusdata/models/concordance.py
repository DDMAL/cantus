...
class Concordance(models.Model):
    ...
    def __str__(self):
        return "{0}  {1}, {2}, {3} ({4}, from {5}) [RISM: {6}]".format(
            ...
            self.library_manuscript_name, self.date,
            self.location, self.rism_code)
...
def solr_index(sender, instance, created, **kwargs):
    ...
    record = solrconn.query("type:cantusdata_concordance item_id:{0}".format(instance.id),
                            q_op="AND")
    if record:
    ...
    d = {
        ...
        'concordance_citation': concordance.__str__()
        # 'letter_code': concordance.letter_code,
        # 'institution_city': concordance.institution_city,
        # 'institution_name': concordance.institution_name,
        # 'sections': concordance.library_manuscript_name,
        # 'date': concordance.date,
        # 'location': concordance.location,
        # 'rism_code': concordance.rism_code
    }
    solrconn.add(**d)
    solrconn.commit()
...
def solr_delete(sender, instance, **kwargs):
    ...
    import solr
    solrconn = solr.SolrConnection(settings.SOLR_SERVER)
    record = solrconn.query("type:cantusdata_concordance item_id:{0}".format(instance.id), q_op="AND")
    if record:
        solrconn.delete(record.results[0]['id'])
        solrconn.commit()
